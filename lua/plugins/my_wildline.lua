...
basic.vi_mode_sep_left = {
    ...
    text = function() return {{sep.right_filled, state.mode[2]}} end
}
    ...
    text = function()
        if hide_in_width() and lsp_comps.check_lsp() then
            return {
                ...
                {
                    lsp_comps.lsp_error(
                        {format = 'errors %s', show_zero = false}), 'red'
                },
                {
                    lsp_comps.lsp_warning(
                        {format = ' warnings %s', show_zero = false}), 'yellow'
                },
                {
                    lsp_comps.lsp_hint({format = ' hints %s', show_zero = false}),
                    'blue'
                }
            }
        end
...
basic.right = {
    ...
    text = function()
        return {
            {' ', state.mode[2]}, {b_components.progress, ''}, {'', ''},
...
basic.right_sep = {
    hl_colors = colors_mode_light,
    text = function() return {{sep.left_filled, state.mode[2]}} end
}
local default = {
    filetypes = {'default'},
    active = {
        basic.vi_mode, basic.vi_mode_sep_left,
        {git_comps.git_branch({}), {'white_light', 'black_light'}},
        {' ' .. sep.right .. ' ', ''}, {b_components.file_name(), ''},
        {sep.right_filled, {'black_light', 'NormalBg'}}, basic.lsp_diagnos,
        {vim_components.search_count(), {"cyan", "NormalBg"}}, basic.divider,
        {sep.left_filled, {'black_light', 'NormalBg'}},
        {' ', {'white_light', 'black_light'}},
        {b_components.file_type({icon = true}), ''}, {' ', ''},
        {b_components.file_encoding(), ''}, {' ', ''},
        {
            b_components.file_format({icon = false}),
            {'white_light', 'black_light'}
        }, {' ', ''}, basic.right_sep, basic.right
    },
    inactive = {
        {b_components.full_file_name, hl_list.Inactive}, basic.divider,
        basic.divider, {b_components.line_col, hl_list.Inactive},
        {b_components.progress, hl_list.Inactive}
    }
}
-- 􏾾􏾿􏿀􏿁􏿂􏿃
windline.setup({statuslines = {default}})
