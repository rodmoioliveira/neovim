...
basic.lsp_diagnos = {
    ...
    text = function()
        if hide_in_width() and lsp_comps.check_lsp() then
            return {
                { ' ', 'red' },
                { lsp_comps.lsp_error({ format = 'errors %s', show_zero = false }), 'red' },
                { lsp_comps.lsp_warning({ format = ' warnings %s', show_zero = false }), 'yellow' },
                { lsp_comps.lsp_hint({ format = ' hints %s', show_zero = false }), 'blue' },
            }
        end
        return { ' ', 'red' }
    end,
...
local default = {
    ...
    active = {
        ...
        { ' ', '' },
        { b_components.file_format({ icon = false }), { 'white_light', 'black_light' } },
        { ' ', '' },
        basic.right_sep,
        basic.right,
    },
