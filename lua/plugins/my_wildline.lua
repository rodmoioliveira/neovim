...
basic.section_x = {
    ...
    text = function(_, _, width)
        if width > width_breakpoint then
            return {
                {sep.left_filled, state.mode[2] .. 'Sep'}, {' ', state.mode[2]},
    ...
    text = function(_, _, width)
        if width > width_breakpoint then
            return {
                ...
                {b_components.cache_file_type({icon = false}), state.mode[2]},
                {' '}
            }
        end
        return {{sep.left_filled, state.mode[2] .. 'Sep'}}
...
basic.section_z = {
    ...
    text = function(_, _, width)
        if width > width_breakpoint then
            return {
                {sep.left_filled, state.mode[2] .. 'Sep'}, {' ', state.mode[2]},
                {b_components.progress_lua}, {' '}, {b_components.line_col_lua}
            }
        end
        return {
            {sep.left_filled, state.mode[2] .. 'Sep'}, {' ', state.mode[2]},
    ...
    text = function(bufnr)
        if lsp_comps.check_lsp(bufnr) then
            return {
                {
                    lsp_comps.lsp_error(
                        {format = ' errors %s', show_zero = true}), 'red'
                },
                {
                    lsp_comps.lsp_warning(
                        {format = ' warnings %s', show_zero = true}), 'yellow'
                },
                {
                    lsp_comps.lsp_hint({format = ' hints %s', show_zero = true}),
                    'blue'
                }
            }
        end
...
local default = {
    filetypes = {'default'},
    active = {
        basic.section_a, basic.section_b, basic.section_c, basic.lsp_diagnos,
        {vim_components.search_count(), {'cyan', 'NormalBg'}}, basic.divider,
        basic.git, basic.section_x, basic.section_y, basic.section_z
    },
    inactive = {
